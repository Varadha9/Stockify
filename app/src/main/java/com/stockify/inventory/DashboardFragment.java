...
public class DashboardFragment extends Fragment {
    ...
    private void loadStats() {
        ...
        AppExecutor.get().execute(() -> {
            ...
            int distinctItems = db.inventoryDao().getDistinctItemCount();
            ...
            requireActivity().runOnUiThread(() -> {
                if (!isAdded()) return;
                txtItemsInStock.setText(String.format(java.util.Locale.getDefault(), "%d", distinctItems));
    ...
    private void renderTrend(List<ValueSnapshot> snapshots) {
        ...
        if (snapshots.isEmpty()) {
            trendChart.clear();
            trendChart.setNoDataText(getString(R.string.empty_trend_data));
