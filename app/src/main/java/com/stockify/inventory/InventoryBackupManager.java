...
public final class InventoryBackupManager {
    ...
    public static void writeBackup(Context context, Uri uri, BackupData data)
        ...
        for (InventoryItem item : data.items) {
            ...
            obj.put("costPrice",         item.getCostPrice());
            obj.put("reorderQuantity",   item.getReorderQuantity());
            items.put(obj);
        }
    ...
    public static BackupData readBackup(Context context, Uri uri)
        ...
        if (items != null) {
            for (int i = 0; i < items.length(); i++) {
                ...
                item.setCostPrice(obj.optDouble("costPrice", 0.0));
                item.setReorderQuantity(obj.optInt("reorderQuantity", 0));
                data.items.add(item);
            }
        }
