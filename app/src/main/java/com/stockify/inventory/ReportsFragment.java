...
public class ReportsFragment extends Fragment {
    ...
    private void loadReport(View view) {
        ...
        TextView txtCostValue = view.findViewById(R.id.report_cost_value);
        ...
        AppExecutor.get().execute(() -> {
            ...
            for (InventoryItem item : items) {
                ...
                totalCostValue += item.getCostPrice() * item.getQuantity();
            ...
            final double finalTotalCostValue = totalCostValue;
            ...
            requireActivity().runOnUiThread(() -> {
                ...
                if (txtCostValue != null)
                    txtCostValue.setText(CurrencyFormatter.format(requireContext(), finalTotalCostValue));
    ...
    private void renderBreakdown(LinearLayout container, Map<String, Integer> values, String suffix) {
        ...
        if (values.isEmpty()) {
            container.addView(simpleText(getString(R.string.report_no_data), R.color.text_hint, 14));
            return;
        }
    ...
    private void renderActivity(LinearLayout container, List<StockLog> logs) {
        container.removeAllViews();
        if (logs.isEmpty()) {
            container.addView(simpleText(getString(R.string.report_no_activity), R.color.text_hint, 14));
            return;
        }
    ...
    private void renderPieChart(PieChart chart, Map<String, Integer> values) {
        if (values.isEmpty()) {
            chart.clear();
            chart.setNoDataText(getString(R.string.report_no_category_data));
            return;
        }
    ...
    private void renderBarChart(BarChart chart, List<InventoryItem> items) {
        if (items.isEmpty()) {
            chart.clear();
            chart.setNoDataText(getString(R.string.report_no_stock_data));
            return;
        }
        ...
        int max = Math.min(8, sorted.size());
        for (int i = 0; i < max; i++) {
