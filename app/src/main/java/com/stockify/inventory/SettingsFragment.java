...
public class SettingsFragment extends Fragment {
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.btn_delete_account).setOnClickListener(v -> confirmDeleteAccount(prefs));
    ...
    private void confirmRestore(Uri uri) {
        new AlertDialog.Builder(requireContext())
                .setTitle(R.string.dialog_confirm_restore_title)
                .setMessage(R.string.dialog_confirm_restore_message)
                .setPositiveButton(R.string.btn_restore, (dialog, which) -> restoreJson(uri))
    ...
    private void showChangePasswordDialog(SharedPreferences prefs) {
        ...
        new AlertDialog.Builder(requireContext())
                ...
                .setView(layout)
                .setPositiveButton(R.string.btn_update, (dialog, which) -> {
                    ...
                    if (!PasswordUtils.verifyPassword(current, savedHash)) {
                        Toast.makeText(requireContext(), R.string.error_current_password_wrong, Toast.LENGTH_SHORT).show();
                        return;
                    }
                    if (newPass.length() < 6) {
                        Toast.makeText(requireContext(), R.string.error_new_password_short, Toast.LENGTH_SHORT).show();
                        return;
                    }
                    if (!newPass.equals(confirm)) {
                        Toast.makeText(requireContext(), R.string.error_passwords_no_match, Toast.LENGTH_SHORT).show();
                        return;
                    }
    ...
    private void confirmDeleteAccount(SharedPreferences prefs) {
        new AlertDialog.Builder(requireContext())
                .setTitle(R.string.dialog_delete_account_title)
                .setMessage(R.string.dialog_delete_account_message)
                .setPositiveButton(R.string.btn_delete, (dialog, which) -> deleteAccount(prefs))
                .setNegativeButton(R.string.btn_cancel, null)
                .show();
    }

    private void deleteAccount(SharedPreferences prefs) {
        AppExecutor.get().execute(() -> {
            InventoryDatabase db = InventoryDatabase.getDatabase(requireContext());
            db.runInTransaction(() -> {
                db.inventoryDao().deleteAll();
